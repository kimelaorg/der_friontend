...
      .pipe(
        tap(response => console.log('Customer Created:', response.id)),
        switchMap(response => of(response.id)),
        catchError(err => {
          console.error('Customer Creation Failed during final submit:', err);
          this.errorMessage.set(err.error?.message || 'Failed to create customer. Proceeding with anonymous sale.');
          return of(null);
        })
      );
  }
...
    }));
    ...
    const finalPayload = {
        customer_id: customerId, // Nullable (null if skipped or creation failed)
        sales_outlet: null, // Set to null as it's nullable and not being collected

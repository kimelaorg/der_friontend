...
import { Observable, of, throwError, Subscription } from 'rxjs'; // Added Subscription
import { debounceTime, switchMap, catchError, finalize, tap, map, distinctUntilChanged } from 'rxjs/operators'; // Added distinctUntilChanged

// --- API Endpoints ---
const API_BASE = 'http://127.0.0.1:8000/api';
const PRODUCT_SPEC_API = `${API_BASE}/sales/products/specs/`;
...
export class Sales implements OnInit {
  ...
  // ... (Component Properties remain the same) ...
  ...
  // RxJS Subscription to clean up the search stream
  ...
  ngOnInit(): void {
    ...
    // REINSTATED: Call the setup method
    this.setupAutocomplete();
  }

  ngOnDestroy(): void {
    // IMPORTANT: Unsubscribe when the component is destroyed
  ...
  // --- Stage 1: Customer Details (Methods kept as is) ---
  ...
  // --- Stage 2: Sales/Items (Autocomplete Logic) ---

  /**
   * Subscribes to changes in the search input and manages the API call stream.
   * This logic is correct for your custom HTML autocomplete list.
   */
  setupAutocomplete(): void {
    this.searchSubscription = this.salesForm.get('searchQuery')?.valueChanges
      .pipe(
        ...
        debounceTime(300),
        // 2. Only search if the term has changed
        distinctUntilChanged(),
        // 3. Switch to the API call, cancelling any previous slow call
      ...
      .subscribe(results => {
        // 5. Update the component property which drives the *ngFor in the template
        this.autocompleteResults = results;
      });
  }

  searchProducts(query: string): Observable<ProductSpecification[]> {
    // Return empty array Observable if the query is too short or empty
  ...
  /**
   * Handles selection of a product from the custom <ul> list.
   */
  selectProduct(product: ProductSpecification): void {
    console.log('Product selected from list:', product);

    // Get the quantity from the form control
    const quantity = this.salesForm.get('quantity')?.value || 1;
    this.addItemToCart(product, quantity);

    // Clear the search field and hide the results list
    this.salesForm.get('searchQuery')?.setValue('', { emitEvent: false }); // Avoid triggering a new search event
    this.autocompleteResults = [];
  }
  ...
  addItemToCart(product: ProductSpecification, quantity: number): void {
    ...
    const priceString = product.discounted_price;
    ...
    const newItem: CartItem = {
      ...
      unit_measure: product.unit_measure || 'PCS',
  ...
  // ... (Other cart/utility methods kept as is) ...
